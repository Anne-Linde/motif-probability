...
motif_probability <- function(motif, hsmm) {
  ...
  alpha[, 1] <- log(init_probs) + motif[1,]$length * log(emission_prob) + log(duration_prob)
  ...
  if(n_events > 1){ # if motif has multiple events
    for (t in 2:n_events) { # For each time step
      ...
      for (j in 1:n_states) { 
        ...
        alpha[j, t] <- log(sum_previous_alphas) + motif[t,]$length * log(emission_prob[j]) + log(duration_prob[j])
      }
    }
  }
